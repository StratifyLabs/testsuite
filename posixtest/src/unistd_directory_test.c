...
int mkdir_test() {
  ...
  if (mkdir("/home/nosearch/test",
            0777)) { // /home/nosearch is not searchable so this should fail
    if (errno != EEXIST) {
      fflush(stdout);
      perror("failed");
      return -1;
  ...
  if (mkdir("/home/nosearch/test1",
            0777)) { // /home/nosearch is not searchable so this should fail
    if (errno != EACCES) {
      printf("Failed to detect permissions error in creating "
             "/home/nosearch/test (%d)\n",
             errno);
      return -1;
    }
  }
